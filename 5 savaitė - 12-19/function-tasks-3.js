console.group("28. Sukurkite funkciją, kuri atspausdina žodį atvirkščiai 'abc' -> 'cba'");
{
  function strReverse(str) {
    const reverseString = Object.assign([], str).reverse().join("");
...
console.group("30. Sukurkite funkciją, kuri taiso pastraipos klaidas");
{
  ...
  function reduceEmptySpaces(str) {
    ...
    const reduced = str.replaceAll(/\s([.,!?])/g, "$1");
    return reduced.replaceAll(/(\s+)/g, " ").trim();
  }
  // 1.
  function fixParagraph(paragraph) {
    ...
    */
    ...
    let fixedParagraph = "";
    const splitSentences = splitIntoSentences(paragraph);
    const reduceSentences = splitSentences.newSentenceArray.map((str) => reduceEmptySpaces(str));
    const capitalizeFirst = reduceSentences.map((str) => capitalizeFirstLetter(str));
    capitalizeFirst.forEach((_, i) => {
      fixedParagraph += capitalizeFirst[i];
      fixedParagraph += splitSentences.newSeparatorArray[i] + " ";
    });
    // for (let i = 0; i < capitalizeFirst.length; i++) {
    //   fixedParagraph += capitalizeFirst[i];
    //   fixedParagraph += splitSentences.newSeparatorArray[i] + " ";
    // }
