...
{
  electricCars = cars.filter(({ fuelTypes }) => fuelTypes.includes("electric"));
...
{
  petrolCars = cars.filter(({ fuelTypes, year }) => fuelTypes.includes("petrol") && year > 2016);
...
console.group("7. Atrinkite audi tipo automobilius nuo 2012 iki 2016, kurie nėra dyzeliniai");
{
  const selectedCars = cars.filter(
    (car) => car.brand.toLowerCase() === "audi" && car.year > 2012 && car.year < 2016 && !car.fuelTypes.includes("dysel")
  );
